...
public final class StringUtils
{
    ...
    public static String getSVNIpAddress()
    {
        WifiManager wifiManager = (WifiManager) CCAPP.getInstances().getApplication()
                .getSystemService(Context.WIFI_SERVICE);
        if (wifiManager == null)
...
//        }
        try
        {
            ...
            for (Enumeration<NetworkInterface> en = networkInfo; en.hasMoreElements();)
            {
                ...
                String intfName = intf.getName().toLowerCase();
                if(!intfName.equals("ppp0") && !intfName.equals("tun0"))
                {
                    continue;
                }
                for (Enumeration<InetAddress> enumIpAddr = intfAddress; enumIpAddr.hasMoreElements();)
                {
                    ...
                    if (!inetAddress.isLoopbackAddress())
                    {
                        ...
                        if (isIPV4Addr(ip))
                        {
                            if (intfName.equals("ppp0"))
                            {
                                LogUtil.d(TAG, "ppp0 -> " + ip);
                            }
                            else if (intfName.equals("tun0"))
                            {
                                LogUtil.d(TAG, "tun0 -> " + ip);
                            }
                            return ip;
                        }
                    }
