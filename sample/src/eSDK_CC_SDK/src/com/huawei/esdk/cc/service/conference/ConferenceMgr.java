...
//                        viewGroupSetVideoparam.addView(surfaceViewSetParam);
    ...
    */
    ...
    public boolean chatSendMsg(int nDstID, String strMsg)
    {
        ...
        int nRet = conf.confSendData(serviceConfig.getFromUserId(), 25, strMsg.getBytes());
        LogUtil.d(TAG, "confSendData nRet == " + nRet);
        return (nRet == 0);
    }
    ...
    public void confMsgNotify(ConfMsg msg, ConfExtendMsg extendMsg)
    {
        ...
        switch (msgType)
        {
            ...
            case ConfMsg.COMPT_MSG_CHAT_ON_RECV_MSG:
                ...
                ConfExtendChatMsg msg1 = (ConfExtendChatMsg)extendMsg;

                byte[] msgInfo = msg1.getMsgInfo();
                String receiveString = null;
                try
                {
                    receiveString = new String(msgInfo, "UTF-8");
                }
                catch (UnsupportedEncodingException e)
                {
                }
                LogUtil.d(TAG, "COMPT_MSG_CHAT_ON_RECV_MSG receive msg: " + receiveString);
                ...
                // 设备添加或是删除:(包括自己和别人)
            {
                if (extendMsg != null)
                {
                    ...
                    if (fromMsg != null)
                    {
                        ...
                        serviceConfig.setFromUserId(fromMsg.getUserId());
                        sendConferenceInfo(serviceConfig.getClientDeviceid());
                    }
            ...
            case ConfMsg.COMPT_MSG_VIDEO_ON_SWITCH: // 视频状态相关:1:打开 0:关闭 2:Resume
                ...
                if (nValue1 == 1 || nValue1 == 2) // 1:打开 2:Resume
                {
                    if (nValue2 == AddressTools.parseInt(serviceConfig
                            .getUserId())) // 本地
                    {
                        serviceConfig.setClientDeviceid(deviceid);
    ...
    private void sendConferenceInfo(String deviceId)
    {
        JSONObject jObject1 = new JSONObject();
        JSONObject jObject2 = new JSONObject();

        try {
            jObject2.put("ConferenceId",serviceConfig.getUserId());
            jObject2.put("isThirdParty",false);
            jObject2.put("IsUser",true);
            jObject2.put("PhoneId","");
            jObject2.put("UserId",serviceConfig.getUserId());
            jObject2.put("extDeviceId","");
            jObject2.put("mainDeviceId",deviceId);

            jObject1.put("Address",jObject2);
            jObject1.put("CallType","1");
            jObject1.put("IsAnonymous",true);
            jObject1.put("MediatypeInfo","1,2,3");
            jObject1.put("VideoType","1");
            LogUtil.d(TAG,jObject1.toString());

            chatSendMsg(0,jObject1.toString());
